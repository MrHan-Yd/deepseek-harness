---
description: "Web @file、@session 与 #session 引用 source：候选项、排序，以及原子行内引用。"
...
Web 用户需要从同一个 `@` 补全菜单提及文件、文件夹或会话，或从 `#` 菜单只提及主会话时，可以使用 `dsh-client-ui-reference`。菜单先列出文件，再列出会话；其中一组无法加载时，另一组仍然可用；`#` 按工作区树的口径剔除子代理，再把当前工作区的会话排在最前、其他工作区排在最后。选择文件、文件夹或会话会插入带稳定剪贴板形式的原子引用；文件夹行还允许用户在不关闭补全的情况下继续下钻。文件行省略多余的根目录位置，会话行仅在工作区与当前工作区不同时显示该工作区。会话 mention 会在捕获模型上下文前接受校验，而浏览候选项不会影响模型。
...
只要组合挂载了本包且存在宿主 `ctx.fileReferences` 提供方，这些 source 即处于活动状态。输入 `@` 后跟一个未加引号的 token，会先看到文件、再看到会话；打开 `@"…` 则只搜索文件。输入 `#` 只搜索主会话，当前工作区排在最前。候选列表是补全菜单，不是搜索结果页：选一次、继续输入即可。
...
`#` token 只运行会话查询，永远不列出文件。它保留工作区树所根植的那些行——列表报告为 subagent 的 Session 会被剔除，因为 `@` 已经能通过其父会话触及本 Session 自己的子代理——然后把 resolver 的工作区亲和排序（该排序本就把发起会话的工作区放在最前）划分为「当前工作区」与「其他工作区」两组。两组在 locale 管理的分组标题下渲染，组内保持原有排序。
...
**运行时不变式：** 不发布伴生入口。插件注册两个 slash source，HMR 测试覆盖释放；它们不发出 Cordis 事件，也不持有跨插件可变状态。
