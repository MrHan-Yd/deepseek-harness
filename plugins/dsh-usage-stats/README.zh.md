...
| Token 活动 | 53 周热力图，支持 每日 / 每周 / 累计 三种口径。**只有真正产生过消耗的那一天才会点亮**；口径决定的是颜色刻度——当天、当周、还是累计——所以一周里只用了两天就只亮两格，不会是七格。悬停任一格显示当天日期，下一行是当天的 Token 数与已结束轮次数（`1.5亿 tokens · 66 轮消息`），非「每日」时再接上该口径的总量 |
| 时间范围 | 近 7 日 / 近 30 日，决定下方趋势图的范围 |
| 每日 Token 趋势图 | 选定范围内每个模型一条线；排名前六之外的模型合并成一条「其他」。悬停画出该日竖线，卡片标题为 `<月日> - <当天总量> tokens`，下面按当天消耗从高到低列出当天有消耗的模型（色点、名称、当日 Token 数） |
| 模型用量 | 环形图展示各模型占比，并列出百分比与 Token 数。悬停任一扇区或图例项：色点后是该模型名，下面一行给出 `<该模型 tokens> <占比>`，并把其余项压暗 |
...
| 路由 | 返回 |
|---|---|
| `GET /usage-stats/api/summary` | 合计、按天分桶（Token、按模型的 Token，以及已结束轮次）、按模型行，以及每个会话一行 |
...
`tests/fold.test.mjs` 用一份内存中的会话语料固定住归属规则——模型切换、替换、重试、stream 分片里的 usage、本地日期分桶、按天的轮次计数、会话跨度、跳过的会话、折算缓存的复用与回收，以及信任防线。`tests/charts.client.test.mjs` 在 jsdom 里带一份桩数据启动浏览器半边，驱动三处悬停读数：热力图格子报出当天日期与当天的 Token/轮次，趋势图的日期带以当天总量为标题并列出当天有消耗的模型，环形图扇区报出自己的 Token 与占比。
